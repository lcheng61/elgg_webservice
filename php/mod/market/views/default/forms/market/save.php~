...
echo "<div><label>";
echo elgg_echo('title') . "<span class='elgg-subtext mlm'>" . elgg_echo("market:title:help") . "</span>";
...
if (!empty($marketcategories)) {
	...
	echo elgg_echo('market:categories:choose') . "</label></div>";
}
...
echo elgg_echo('market:type:choose') . "</label></div>";
...
if (elgg_get_plugin_setting('market_custom', 'market') == 'yes' && !empty($custom_choices)) {
	...
	echo elgg_echo('market:custom:select') . "</label></div>";
}

echo "<div><label>" . elgg_echo("market:text");
if ($allowhtml != 'yes') {
	echo "<span class='elgg-subtext mlm'>" . elgg_echo("market:text:help", array($numchars)) . "</span>";
	echo "<textarea name='description' class='mceNoEditor' rows='5' cols='40' onKeyDown='textCounter(document.marketForm.description,document.marketForm.remLen1,{$numchars}' onKeyUp='textCounter(document.marketForm.description,document.marketForm.remLen1,{$numchars})'>{$vars['description']}</textarea><br />";
...
echo "<div><label>" . elgg_echo("market:price");
...
echo "<span class='elgg-subtext mlm'>" . elgg_echo("market:imagelimitation") . "</span>";
echo "</label></div>";
...
$body1 = "<div><label>" . elgg_echo("market:uploadimage1");
if ($images[1]) {
	$body1 .= elgg_view('output/confirmlink', array(
			...
			'confirm' => elgg_echo('market:delete:image'),
			));
}
...
$body2 = "<div><label>" . elgg_echo("market:uploadimage2");
if ($images[2]) {
...
$body3 = "<div><label>" . elgg_echo("market:uploadimage3");
if ($images[3]) {
...
$body4 = "<div><label>" . elgg_echo("market:uploadimage4");
if ($images[4]) {
...
echo elgg_echo('access') . "<span class='elgg-subtext mlm'>" . elgg_echo("market:access:help") . "</span></label></div>";
