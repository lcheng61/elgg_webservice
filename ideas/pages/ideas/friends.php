<?php
/**
 * Elgg Ideas Plugin
 * @package ideas
 */
		
// Get input
$offset = get_input('offset', 0);
$owner_name = get_input('username');
$owner = get_user_by_username($owner_name);

elgg_push_breadcrumb(elgg_echo('ideas:title'), "ideas/category");
elgg_push_breadcrumb($owner->name, "ideas/owned/{$owner->username}");
elgg_push_breadcrumb(elgg_echo('friends'));

elgg_register_title_button();

//set the title
if ($owner->guid == elgg_get_logged_in_user_guid()) {
	$title = elgg_echo('ideas:friends:title');
} else {
	$title = elgg_echo('ideas:user:friends:title', array($owner->name));
}

if (!$friends = get_user_friends($owner->guid, ELGG_ENTITIES_ANY_VALUE, 0)) {
	$content = elgg_echo('friends:none:you');
} else {
	$options = array(
			'type' => 'object',
			'subtype' => 'ideas',
			'full_view' => false,
			'pagination' => true,
			'limit' => 5,
			'view_type_toggle' => FALSE,
			);

	foreach ($friends as $friend) {
		$options['container_guids'][] = $friend->getGUID();
	}

	$content = elgg_list_entities_from_metadata($options);

}

		
if (empty($content)) {
	$content = elgg_echo('ideas:none:found');
}
		
// Show ideas sidebar
$sidebar = elgg_view("ideas/sidebar");

$params = array(
		'filter' => false,
		'content' => $content,
		'title' => $title,
		'sidebar' => $sidebar,
		);

$body = elgg_view_layout('content', $params);

echo elgg_view_page($title, $body);
